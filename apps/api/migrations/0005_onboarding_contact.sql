-- Contact on the draft lets Luminix follow up abandoned signups.
-- Email/phone stay on the payload; they are not a client CRM or WhatsApp grant.
...
ALTER TABLE luminix.onboarding_drafts ADD CONSTRAINT onboarding_drafts_step_check
  CHECK (step IN ('contact', 'clinic', 'occupations', 'services', 'professionals', 'review'));
ALTER TABLE luminix.onboarding_drafts ALTER COLUMN step SET DEFAULT 'contact';
...
BEGIN
  IF p_clinic IS DISTINCT FROM nullif(current_setting('luminix.clinic_id', true), '')::uuid
    ...
    OR p_step IS NULL OR p_step NOT IN ('contact','clinic','occupations','services','professionals','review')
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog AS $$
DECLARE v_draft luminix.onboarding_drafts%ROWTYPE; v_name text; v_owner text; v_email text; v_phone text; v_item jsonb;
BEGIN
  IF p_clinic IS DISTINCT FROM nullif(current_setting('luminix.clinic_id', true), '')::uuid
  ...
  v_owner := trim(v_draft.payload->>'ownerName');
  v_email := lower(trim(v_draft.payload->>'email'));
  v_phone := trim(v_draft.payload->>'phone');
  IF v_name IS NULL OR length(v_name) NOT BETWEEN 1 AND 160
    OR v_owner IS NULL OR length(v_owner) NOT BETWEEN 1 AND 120
    OR v_email IS NULL OR v_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'
    OR v_phone IS NULL OR v_phone !~ '^\+[1-9][0-9]{7,14}$'
